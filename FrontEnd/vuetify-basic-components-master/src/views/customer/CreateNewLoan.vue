  ...
  <v-container fluid>
    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <h1>you are applying for a Loan</h1>

         <h2 class="mt-5"> Interest Rate: {{Data.activeInterestRate}} </h2>
        ...
        ></v-text-field>
         <h2 class="mt-5"> Please note that The Max is {{Data.max}} and the Min is {{Data.min}}  </h2>
          ...
          :rules="retrunDateRules()"
        ></v-text-field>
          <Amortization url="http://127.0.0.1:8000/amortization" :amount="amount" :duration="Data.duration" :interestRate="Data.activeInterestRate" :key="amount" />
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      fetch('http://127.0.0.1:8000/loan', requestOptions)
        .then(response => {
          response.json().then(data => {
            console.log(data)
            if (response.ok) {
